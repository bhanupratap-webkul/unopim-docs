...
```bash
docker pull webkul/unopim:v2.0.0
```

#### Step 2: Run the Container

```bash
docker run -it -d --name unopim_container -p 80:80 webkul/unopim:v2.0.0
```
...
docker run -it -d --name unopim_container -p 8082:80 webkul/unopim:v2.0.0
```
...
* Ensure that **`/run/php/php8.3-fpm.sock`** matches the socket path defined in **`/etc/php/8.3/fpm/pool.d/www.conf`**.
...
server {
    ...
    location ~ \.php$ {
        ...
        fastcgi_pass unix:/run/php/php8.3-fpm.sock;   # Adjust PHP-FPM socket if needed
...
* Make sure PHP-FPM is running (`php8.3-fpm` by default).
